...
const NavBar = ({ logout, updateSelectedUserToCurrentUser, user }) => {
    return(
      <nav>
        <h1>Wax Global</h1>
            {user !== null ? (
            ...
            <button onClick={()=> updateSelectedUserToCurrentUser()}><NavLink to={`/users/${user.id}`} exact >My Profile</NavLink></button>
